<template>
   <div class="menu_left cflex" :style="{width:sidebar.width+'px'}">
        <div class="menu_page_top rflex" @click="logout">
                    ...
                    <template v-for="(item,index) in permission_routers">
                        ...
                        <router-link v-if="!item.hidden && item.noDropdown" :to="item.path+'/'+item.children[0].path" :key="index">
                            <el-menu-item class="dropItem" 
                                ...
                                >
                                <i :class="item.meta.icon" v-if='item.name=="userManage"'></i>
                                <icon-svg v-else :icon-class="item.meta.icon" />
...
export default {
  ...
  mounted(){
    console.log('routes-permission', this.permission_routers);
  },
  methods: {
